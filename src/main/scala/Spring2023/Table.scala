...
case class Table[T](xs: List[T]) {
   ...
   */
  def size: Int = /** SOLUTION END */ xs.length
  ...
  def lensFilter[P](p: P => Boolean)(lens: T => P): Table[T] = /** SOLUTION END */ {
    val filteredXs = xs.filter(t => p(lens(t)))
    Table(filteredXs)
  }
  ...
  def sample(n: Int)(implicit r: Random): Table[T] = /** SOLUTION END */ {
    val filteredXs = xs.filter(_ => r.nextInt(n) == 0)
    Table(filteredXs)
  }
...
object Table {
   ...
   * @return a Table[T].
   */
  def apply[T](xs: LazyList[T]): Table[T] = Table(xs.toList) /** SOLUTION END */
   ...
   * @return a Table[T].
   */
  def apply[T](xs: T*): Table[T] = Table(xs.toList)/** SOLUTION END */
}
